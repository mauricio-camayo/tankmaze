...
function TankDebugPanel({ label, data, color }: {
  label: string;
  ...
  return (
    <div style={{
      ...
      {data.log && data.log.length > 0 && (
        <div style={{ marginTop: 4, borderTop: '1px solid #2d2d4e', paddingTop: 4 }}>
          {data.log.slice(-3).map((line, i) => (
      ...
      <div>
        ...
        {showDebug && tickData && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
            <TankDebugPanel label={`${nameA} (A)`} data={tickData.tankA} color="#60a5fa" />
            <TankDebugPanel label={`${nameB} (B)`} data={tickData.tankB} color="#f97316" />
          </div>
        )}
      </div>
    </div>
