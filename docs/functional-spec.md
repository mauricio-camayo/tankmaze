...
**Fixed — Speed's move cooldown no longer wastes a stat point at tick granularity (PRIORITIES.md item 252):** the effect column above still describes the raw per-point relationship, but the engine no longer derives a tank's move cooldown from that formula at runtime. Move cooldown is now **looked up from an explicit ticks-between-moves table**, `{5, 4, 3, 2, 1}` for Speed 1 through 5, rather than computed as `500 / speed` and rounded up to the engine's 100ms tick boundary — the old approach put both Speed 3 and Speed 4 at an identical 2-ticks-between-moves duty cycle, so a stat point spent going from Speed 3 to Speed 4 bought zero actual movement benefit.

`{5, 4, 3, 2, 1}` is the tightest possible correction, not an arbitrary retune: it keeps Speed 5's existing 1-tick (100% duty cycle) floor and Speed 1's existing 5-tick ceiling exactly where they were, and — because there is no integer strictly between 2 and 3 — Speed 2 and/or Speed 3 necessarily had to move to make room for five distinct, strictly-decreasing tick counts within that same range. That has a real balance consequence: Speed 2 (3→4 ticks between moves) and Speed 3 (2→3 ticks) now move measurably *slower* than they did under the old, buggy formula, while Speed 1, Speed 4, and Speed 5 are unchanged. Any existing tank sitting at Speed 2 or Speed 3 — including the player-authored "Sniper" tank, currently Speed 2 — will feel this. The table is configurable per environment via a `MOVE_COOLDOWN_TICKS` env var, mirroring §8.1's `COLLISION_DAMAGE_TABLE` convention.

See `docs/technical-spec.md` §4.5 for the full mechanism and the regression test that pins Speed 3 and Speed 4 apart going forward. **This fix is implemented and verified but not yet committed to git, and has not been pushed or deployed** — it exists only in this session's working tree.
...
**Fixed — Game Day name display no longer shows the wrong calendar day for the viewer (PRIORITIES.md item 253):** the platform still auto-appends a date suffix to each Game Day's name (e.g. "Saturday Smackdown · Aug 30"), and the *stored* name — computed server-side in UTC — is deliberately unchanged, since other backend logic (PATCH's base-name-preservation, potential lookup by exact stored name) depends on that stored value staying stable. What changed is what the viewer actually sees: every place a Game Day's name is rendered (`GameDay.tsx`'s page header, `GameDayList.tsx`'s admin list row, `Dashboard.tsx`'s registered-Game-Days card) now **recomputes the date suffix client-side, in the viewer's browser-local timezone** — the same locale-formatting convention every other schedule timestamp on the Game Day page already uses — instead of trusting the server's UTC-only suffix. A viewer in a UTC-negative timezone whose Game Day crosses local midnight within the same UTC calendar day now sees a name that agrees with the rest of the page's rendered times, rather than disagreeing by a full calendar day as before. This is a distinct fix from the multi-day-spanning-name case already handled by item 95 (round-robin and final falling on *different* calendar days) — 95's logic is untouched. Cosmetic only, as before — no effect on actual scheduling, matches, or bracket logic. See `docs/technical-spec.md` §5.5 for the shared `gameDayName.ts` utility and verification. **Fixed and verified but not yet committed to git, and not pushed or deployed.**

**Fixed — editing a Game Day's schedule no longer silently fails to reschedule the actual trigger (PRIORITIES.md item 254):** when an admin edits a Game Day's phase times (the Edit form in `GameDayList.tsx`, backed by `PATCH /gamedays/{id}`), the platform now catches the failure mode that previously let the stored schedule and the real EventBridge trigger drift permanently out of sync with no visible sign of it. Two layers: first, the PATCH is now rejected outright with a validation error if any newly-picked phase time isn't safely far enough in the future — catching the exact case most likely to trigger the underlying race, which is editing a schedule to start *sooner*, before any data is written at all. Second, as defense in depth for the AWS-side failures the upfront check can't catch, if a reschedule still fails after the record is saved, the admin now sees an explicit warning listing which phases may still be running on their old schedule, and the edit form stays open rather than reporting the save as fully complete.

Confirmed live on a real gameday (`1073500c-72c0-4524-9c0f-156380eb0359`, "Nerf or Nothing", 2026-08-29): an admin moved the round robin and final times earlier via Edit, the Game Day page correctly reflected the new times, but the actual EventBridge triggers had silently stayed on the original schedule — the Game Day eventually appeared as generically "stuck" (§10 below) with nothing to indicate the true cause was a failed reschedule. That specific live evidence trail is what this fix closes. See `docs/technical-spec.md` §5.5 for the `upsertSchedule`/`patchGameDay` mechanism and verification. **Fixed and verified but not yet committed to git, and not pushed or deployed** — the platform's live behavior as of this writing is still the silent-failure version described above until this change ships.
