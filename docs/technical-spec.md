...
**Real entropy, not a deterministic byte stream (PRIORITIES.md item 251, functional spec §3.5):** until 2026-08-29, the `wazero.NewModuleConfig()` builder in `packages/backend/internal/wasm/wasm.go` never called `.WithRandSource(...)`. wazero's own documented default for that setting is a deterministic byte source (confirmed in the pinned dependency, `github.com/tetratelabs/wazero v1.7.3`, `config.go` ~line 639-640: *"WithRandSource configures a source of random bytes. Defaults to return a deterministic source."*), so every module instantiation — every tank, every match, ranked or unranked — received the identical fixed byte stream for WASI's `random_get` syscall, which is what seeds Go's `math/rand` global source inside the wasip1 tank binary. Any tank logic that calls `math/rand` (confirmed examples: built-in Randy's `randDir()` wander-direction picker in `packages/testdata/tanks/randy/main.go`, and the player-authored Rammer tank's `hookSide` random fallback) was therefore not random at all — it produced the exact same sequence of "random" decisions on every match, given the same map and spawn points.

**Confirmed live (2026-08-28), before the fix:** two real matches — a round-robin match and a later elimination-bracket rematch, both Rammer v2 (`e0de1ddd-ff23-41da-8ebd-57f90a436fbc`) vs. built-in Randy, both on the fixed `builtin-open` map, gameday `e5c9846f-9aaa-41f9-a025-c1cd2786e970` — were downloaded and diffed tick-by-tick (65 ticks each, matchIds `ff829f60-891f-40ea-8c1c-4d1b1715b96b` and `aefddc39-0c6d-4316-b5ec-c96943a7ec4b`). Every game-state field was byte-for-byte identical across both matches; only the `durationMs` wall-clock timing field (§7) differed, as expected from Lambda scheduling noise.

**Fixed (2026-08-29), commit `95f69b3`:** added `.WithRandSource(crand.Reader)` (`crand "crypto/rand"`) to the `wazero.NewModuleConfig()` chain in `wasm.go` — host-side only, no tank WASM recompilation needed, since entropy is supplied by the platform host rather than baked into each tank's compiled binary. Verified with a throwaway harness loading Randy's WASM twice with identical sensor input — action sequences diverged instead of replaying identically.
...
**Tier/placement display (PRIORITIES.md item 249):** `bracketTiers`/`tierPlacement` compute, per tank, the correct ordinal placement for a tie group (1, 2, 3, 5, 9, … via `tierPlacement(k) = (1 << (k-2)) + 1` for k≥3) and this value is written onto each `db.Ranking` record's `Placement` field by `PutRanking`. The "Final standings" UI in `GameDay.tsx` (§9) does not read this field — it re-sorts `gameDay.placementPoints` (a `tankId → points` map) client-side and derives its own rank. Rather than exposing `Placement` through the Game Day API, item 249 (fixed 2026-08-29) closed the resulting tie-display gap with an equivalent client-side `competitionRanks` ("1224") helper (`packages/frontend/src/utils/ranking.ts`), applied to both the round-robin and Final Standings tables — so both computations independently produce the same tie-aware ranks rather than the frontend reading the backend's `Placement` field directly.
...
2. For each due series, checks whether an *earlier* occurrence of that same series has already been materialized and hasn't finished yet — its Final phase status isn't `"complete"` or `"cancelled"` — if so, this tick skips it and waits, so at most one occurrence per series is ever outstanding regardless of how far `nextOccurrenceAt` sits inside the lead-time window (item 262 — a daily series polled hourly used to advance and re-materialize on every tick until it outran the whole 7-day cutoff). This check is on the occurrence's *actual phase status*, not its scheduled round-robin timestamp — round-robin starting doesn't mean the occurrence is done, since elimination rounds and the final still run afterward; gating on the timestamp instead would let the next occurrence materialize while the current one is still running. Otherwise it materializes the next occurrence — reusing the exact same Game Day creation + EventBridge scheduling logic as `tank-api`'s `POST /gamedays` handler (factored into a shared `internal/scheduling` package so both call sites stay identical).
